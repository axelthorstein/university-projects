...
\begin{solution}
	...
	then the graph $G$ is outerplanar.
...
\end{problem}
\begin{solution}
	a) Let $a_v = B =1$ for all $v$. The only independet sets in $K_n$ are sets with one vertex, so this shows, that $K_n$ is a threshold graph. \\
	...
	c) Case 1: All values could be chosen as 0, so all n vertices before are isloated. For each of these vertices set $a_v=1$. For the new vertex set $a_v=n+1$ and set $B=n+2$. Therefor it is threshold. \\
...
\end{problem}
\begin{solution}
	We show that for an arbitray vertex, all other vertices that have maximum distance of two in the original graph are adjacent with the new representation. Therefor, we show that they intersect the nwe interval. \\
	Wlog we can look at the vertex, represented by the interval $[a_1, b_1]$. The old neighbors are included easily because $b_i' \geq b_i$. So take a vertex with distance two to this vertex. Either the interval is completely on the left or the right of $[a_1, b_1]$. If it is on the right, there exists a vertex representing interval, that intersects both of them. Therefor $b_1'$ is high enough, so this two intervals intersect. For an interval completely on the left, the statement is the same, just with the changing $b_i'$ of the other interval. In total it is a representation og $G^2$.
\end{solution}
...
\end{problem}
\begin{solution}
	If there is claw in L(G), the center vertex of this claw has 3 neighbors. An edge is ending in maximum 2 vertices, so in G two of the adjacent edges need to share a vertex. This would mean, they are connected as well, which is a contradiction to the definition of a claw. Therefor, L(G) is claw-free
\end{solution}
...
\end{problem}
\begin{solution}
	We have already shown the upper bound for $\chi(G)$by greedy coloring in th lecture. If $\Delta(G) \leq 2$, the claim is trivial, because you need at least one color. So assume $\Delta(G) >2$. Let$v$ be a vertex with maximum degree and let $N(v)$ be the neighborhood of this vertex. The induced graph $ [ N(v)] $ can maximum contain two cliques, because otherwise you could choose one vertex out of three cliques, that create a claw. Let the bigger or equal clique be $S$. We can conclude 
