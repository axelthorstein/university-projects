...
\end{problem}
\begin{solution}
	We can show that any tree is a bipartite graph because at any level of the tree there are no nodes connected 
	to other nodes at that level, so if we put all odd numbered levels in one set, and all even numbered levels in another set
	so we have two sets of nodes where each node is only connected to a node in the other set. 
	Therefore every tree is bipartite. 
	From this claim
\end{solution}
